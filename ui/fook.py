...
def tests():
	...
	an = [[[[{} for t in range(ts+1)] for r in range(rs+1)]for grid in range(grids)] for algo in algorithm]



	for aa,a in enumerate(algorithm):
			...
			for j,ra in enumerate(range(1,5)):
				for k,ta in enumerate(range(1,11)):
					# try:
						for l in range(1,2):
							# h,ra,ta,i = 10,1,10,1
							print "./output/"+str(a)+"_"+test_case_file
							test_case_file = str(h)+"_"+str(ra)+"_"+str(ta)+"_"+str(l)
							with open("./output/"+str(a)+"_"+test_case_file) as f: s = f.read()
							a = s.split("\n")[0].split(" ")
							...
							if(len(s)>4):
								...
								oom = (a[-1] == "OOM")
							print tle,oom						
							an[aa][i][j+1][k+1]["tles"] = tle
							an[aa][i][j+1][k+1]["ooms"] = oom
							an[aa][i][j+1][k+1]["exp"] = states
							an[aa][i][j+1][k+1]["mem"] = memory
							an[aa][i][j+1][k+1]["sol"] = soln
					# except Exception:
					# 	an[aa][i][j+1][k+1] = "FAIL"
		...
		for j,ra in enumerate(range(1,5)):
			...
			for k,ta in enumerate(range(1,11)):
				...
				for aa,a in enumerate(algorithm): 
					...
					htm += "<a href='/?input="+fname+"&algo="+a+"&delay=500'>"+"I: "+"</a>"
					print an[aa][i][j+1][k+1]
					if an[aa][i][j+1][k+1] == "FAIL":
						htm +='FAIL'
					elif an[aa][i][j+1][k+1]["tles"] >0:
						htm +='TLE'
					elif an[aa][i][j+1][k+1]["ooms"] >0:
						htm +='OOM'
					else:
						htm += str(an[aa][i][j+1][k+1]["sol"])
					# if an[i][j+1][k+1] != "FAIL":
					# 	htm += " ("+str((an[aa][i][j+1][k+1]["exp"]))+")"
					htm+="<br>"
				htm += "</td>"
			htm += "</tr>"
		htm += "</table>"
