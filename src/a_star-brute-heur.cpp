...
state transition(state s, action a){
    ...
    // cout<< "actual"<<s.actual_<<endl;
    new_state.actual_ = s.actual_+a.cost();
    // cout<< "actual"<<new_state.actual_<<endl;
    // cout<<"-Parent Set as: "<<(*(new_state.parent)).getRep()<< " For:"<<new_state.getRep()<<endl;
    return new_state;
}

//Returns lexicographically 'next' allocation
void nextAlloc(int alloc[], state s){
    int robots=s.robots.size(),i=targets.size()-1;
    while(i){
        if(alloc[i]<robots-1){
            alloc[i]++;
            break;
...
//returns an LB on tsp cost on a grid
int roboheur(state s,vector<int> robot, position p){
    int xmin=p.col,ymin=p.row,xmax=p.col,ymax=p.row;
    for(int i =0; i <robot.size(); ++i){
        xmin=min(xmin,s.robots[i].col);
        ymin=min(ymin,s.robots[i].row);
        xmax=max(xmax,s.robots[i].col);
        ymax=max(ymax,s.robots[i].row);
    }
    return xmax-xmin+ymax-ymin+min(xmax-p.col,p.col-xmin)+min(ymax-p.row,p.row-ymin);
}

//Returns an LBound on cost of an allocation
int AllocCost(state s, int alloc[]){
    int sum=0,r=s.robots.size(),t=targets.size();
    vector<int> robot[r];
    for(int i=0;i<t;i++)
        robot[alloc[i]].push_back(i);
    for(int i=0;i<r;i++)
        sum+=roboheur(s,robot[i],s.robots[i]);
    return sum;
}

// shubhorup
ll heuristic(state s){
    int r=s.robots.size(),t=targets.size(),alloc[t],mini=INT_MAX;
    
    //initial allocation
    for(int i=0;i<t;i++)
        alloc[i]=0;
    
    for(int i=0;i<pow(r,t);i++){
        mini=min(mini,AllocCost(s,alloc));
        nextAlloc(alloc,s);
    }
    return mini;
}
...
pair<state, double> dijkstra(state start_state){
    ...
    while(!Heap.empty())
    {
        ...
        if(memory/1024 > 500){
            ...
            return make_pair(current_state, current_state.actual_);
        }
        if(time_spent>2){
        ...
        if(goal_check(current_state)){
            ...
            cout<<current_state.actual_<<" "<<states_expanded<<" "<<time_spent<<" "<<memory<<" KB\n";
            return make_pair(current_state, current_state.actual_);
        }
        // check if already expanded
            ...
            for (int i = 0; i < actions_.size(); ++i)
            {
                ...
                double f;
                if(targets.size()<7){
                 f = g + heuristic(transitioned_state);
                }else{
                    f =g;
                }
...
int getY(int position, int n){
    return position % n;
}
int getX(int position, int n){
    return (position-getY(position,n))/n;
}
int getPosition(int i, int j, int n){
    return (i*n)+j;
}

pair<state, double> dijkstra_grid(state start_state){
    std::map<int ,double > visited;
    priority_queue<pair<int , double>, vector<pair<int, double> >, Comparator> Heap;
